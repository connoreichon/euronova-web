...
- **MICRO-ANIMACIONES DE PINTURA (pulido):** keyframes/clases en `global.css` (`.sim-wet`, `fav-pop-in`/`fav-shrink-out`, `latido`, `fav-bar-in`), gated con `prefers-reduced-motion`. (1) **Barrido húmedo**: al aplicar color en el simulador, un destello cruza la superficie pintada una vez (capa `.sim-wet`+`.sim-wet__band` por panel, enmascarada; disparado con debounce 200ms en `pintar()` para no repetirse al teclear). (2) **Guardar = salpicadura del color**: `Base.astro` expone `window.salpicarEn(x,y,color,{sprite?,gotas?})` (refactor del splash al clic, mismas gotas WAAPI); al guardar en el simulador o en las cartas, salpica gotas del propio color + el corazón "late". El clic global se salta `.fav-btn`/`#sim-guardar` para no duplicar. (3) **Muestras**: entran con pop y salen encogiéndose en `#sim-favs`. (4) **Barra de cartas** entra con rebote (contenedor flex centra, el `<a>` anima solo la Y). Cableado verificado por DOM.
