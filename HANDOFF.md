...
- **SUPERFICIE** (tabs `[data-surface]`): SOLO **Interior** y **Exterior** (el cliente pidió QUITAR la superficie Esmalte/metal en commit `a0b4e52` — una verja limpia SIN flores no era viable con stock gratis: las bonitas están en jardines con follaje delante que fragmenta el corte).
  - **Interior** (`interior.jpg` = `wall-751342`): a petición del cliente es **SOLO UNA PARED lisa** (con suelo abajo), sin sofá ni muebles. Se pinta solo la pared, nunca el rodapié/suelo. Máscara = rectángulo de la pared por encima de la línea del rodapié (y~611). La base rosada se **neutralizó** (desaturar 65% + aclarar) para que el color salga fiel.
  - **Exterior** (`fachada.jpg` = `door-2096367`, con la placa/cartel clonada fuera): **corte por COLOR** (commit `c04b778`) — se pinta TODO lo blanco/claro y se excluye solo lo AZUL (puerta/ventana: `B-(R+G)/2 > 14`) y lo OSCURO (farol/rejas), por encima de la acera. Así no quedan blancos alrededor de puerta/ventana/farol y el corte sigue la forma exacta. Fue la solución a los "fallitos" (huecos rectangulares dejaban halos blancos).
