...
- **SUPERFICIE** (tabs `[data-surface]`): SOLO **Interior** y **Exterior** (el cliente pidió QUITAR la superficie Esmalte/metal en commit `a0b4e52` — una verja limpia SIN flores no era viable con stock gratis: las bonitas están en jardines con follaje delante que fragmenta el corte).
  ...
  - **Exterior** (`fachada.jpg` = `door-2096367`, con la placa/cartel clonada fuera y **el farol de encima de la puerta ELIMINADO**): **corte por COLOR** (commit `c04b778`) — se pinta TODO lo blanco/claro y se excluye solo lo AZUL (puerta/ventana: `B-(R+G)/2 > 14`) y lo OSCURO (rejas), por encima de la acera. El farol se quitó por **inpaint horizontal** (interpola entre la columna izquierda y derecha de la caja del farol → reconstruye pared blanca arriba y la línea del marco azul abajo); primero el cuerpo grande y luego el **resto/soporte** (óvalo oscuro sobre el listón superior del marco, caja `x168-216, y144-165`). Tras limpiar el farol la **máscara se REGENERÓ** desde la foto limpia (si no, el resto oscuro dejaba un hueco sin pintar). Así no quedan blancos alrededor de puerta/ventana ni restos del farol y el corte sigue la forma exacta. Fue la solución a los "fallitos" (huecos rectangulares dejaban halos blancos).
