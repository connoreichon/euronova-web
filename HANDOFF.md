...
- **"GUARDA TUS COLORES" (favoritos):** sustituye a los antiguos chips fijos "O prueba estos" (petición del cliente). Botón **"Guardar color"** (corazón que se rellena) que **guarda/quita** el color aplicado (`colorActual`); los guardados aparecen como muestras bajo la pared, **clicables para reaplicarlos** y con una **×** para quitarlos. **Persisten en `localStorage`** (clave `euronova:favcolores`, máx. 12, el más nuevo primero) → siguen ahí al volver. Mensaje de estado vacío cuando no hay ninguno. Todo en el `<script>` de `Tintometria.astro` (`renderFavs`/`guardarBtn`/delegación en `#sim-favs`); el corazón y el texto del botón reflejan si el color actual ya está guardado. Verificado el flujo completo (guardar/aplicar/quitar/recarga).
