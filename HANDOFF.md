...
- **MICRO-ANIMACIONES DE PINTURA (pulido):** keyframes/clases en `global.css` (`fav-pop-in`/`fav-shrink-out`, `latido`, `fav-bar-in`), gated con `prefers-reduced-motion`. (1) **Guardar = salpicadura del color**: `Base.astro` expone `window.salpicarEn(x,y,color,{sprite?,gotas?})` (refactor del splash al clic, mismas gotas WAAPI); al guardar en el simulador o en las cartas, salpica gotas del propio color + el corazón "late". El clic global se salta `.fav-btn`/`#sim-guardar` para no duplicar. (2) **Muestras**: entran con pop y salen encogiéndose en `#sim-favs`. (3) **Barra de cartas** entra con rebote (contenedor flex centra, el `<a>` anima solo la Y). NOTA: hubo un "barrido húmedo" al aplicar color en el muro (capa `.sim-wet`) pero **el cliente lo rechazó y se quitó** — no reintroducir sin pedírselo.
