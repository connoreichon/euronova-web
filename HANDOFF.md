...
- **MICRO-ANIMACIONES DE PINTURA (pulido):** lengua visual = "pincelada y colada de pintura en rojo de marca". Keyframes/clases en `global.css`, gated con `prefers-reduced-motion`.
  ...
  - ⚠ ELIMINADOS por decisión del cliente: la **salpicadura de motitas al clicar** (todo el script de `salpicarEn`/`splash5` fuera de `Base.astro`), la salpicadura al guardar, y el "barrido húmedo" al aplicar color en el muro. NO reintroducir sin pedírselo. Los sprites `assets/manchas/splat*` quedan SIN USO (no borrados).
